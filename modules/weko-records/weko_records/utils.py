...
def check_embargo_rights(access_right: str, today, accessrole_date: list = []):
    """
    Determines whether the accessrights value needs to be updated based on the mapped item values and file information, and what value it should be changed to.
    Args:
        access_right (str): The value mapped to the item's accessRight.
        today (date): The current date.
        accessrole_date (list): List of (accessrole, date) tuples for registered files.
    Returns:
    ...
    if any(role == "open_restricted" for role, _ in accessrole_date):
        return True, "restricted access"

    # 2. If there is at least one 'open_date' with a future date, no update required
    if any(role == "open_date" and date and today and date > today for role, date in accessrole_date):
        return False, None

    # 3. If there is at least one 'open_login', set to 'restricted access'
    if any(role == "open_login" for role, _ in accessrole_date):
        return True, "restricted access"
...
def update_embargo_rights(metadata: dict) -> None:
            ...
            if isinstance(data, dict):
                if key in data:
                    ...
                    for item in data['attribute_value_mlt']:
                        found = _get_nested_value(item, '.'.join(keys[keys.index(key):]))
                ...
                found = None
                for item in data:
                    found = _get_nested_value(item, '.'.join([key] + keys[keys.index(key)+1:]))
                    if found is not None:
                        break
                data = found
                break
    ...
    for v in metadata.values():
        if isinstance(v, dict) and v.get("attribute_type") == "file":
            ...
            for data in mlt:
                ...
                if "date" in data and isinstance(data["date"], list) and data["date"]:
            ...
            for data in v:
                if isinstance(data, dict) and data.get("attribute_type") == "file":
                    mlt = data.get("attribute_value_mlt", [])
                    if mlt:
                        for file_data in mlt:
                            date_val = None
                            accessrole_val = file_data.get("accessrole")
                            if "date" in file_data and isinstance(file_data["date"], list) and file_data["date"]:
    ...
    is_update, change_value = check_embargo_rights(access_right_value, today, accessrole_date)
