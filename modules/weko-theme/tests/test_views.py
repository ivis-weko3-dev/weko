...
def test_index_community_settings(client, users):
    ...
    search_setting_display_community_true = {
        "display_control": {
            "display_community": {"status": True}
        }
    }

    with patch("weko_admin.admin.AdminSettings.get", return_value=db_settings):
        with patch("weko_theme.utils.get_search_setting",
                   return_value=search_setting_display_community_true):
            with patch("weko_admin.utils.current_i18n") as mock_i18n:
