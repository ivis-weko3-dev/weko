...
@pytest.fixture()
def test0(app, without_oaiset_signals, schema,db):
    index_name = f"{app.config['SEARCH_INDEX_PREFIX']}records"
    if not current_search.client.indices.exists(index=index_name):
        current_search.client.indices.create(index=index_name)
...
def test_set_with_no_records(app, without_oaiset_signals, db, schema,es_app):
...
def test_empty_set(app, without_oaiset_signals, db,schema, es_app):
...
def test_set_with_records(app, without_oaiset_signals, db, schema,es_app):
    # create extra record
...
def test_search_pattern_change(without_oaiset_signals, db,es_app):
...
# .tox/c1/bin/pytest --cov=invenio_oaiserver tests/test_percolator.py::testpercolate_query -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/invenio-oaiserver/.tox/c1/tmp
def testpercolate_query(es_app):
    ...
    result = percolate_query("test-weko-item-v1.0.0", "item-v1.0.0", "item-v1.0.0", {})
    assert result == []
    with patch("invenio_oaiserver.percolator.ES_VERSION", [7]):
        result = percolate_query("test-weko-item-v1.0.0", "item_v1.0.0", "item_v1.0.0", {})
...
def test_new_percolator(es_app, db, without_oaiset_signals,mocker):
    ...
    es_app.config["INDEXER_DEFAULT_INDEX"] = "test-weko-item-v1.0.0"
    oai = OAISet(id=1,
        spec='test',
        name='test_name',
...
def test_build_cache(instance_path):
    ...
    with app.app_context():
        ...
        # Clear existing data
        db_.session.query(OAISet).delete()
        db_.session.commit()
