...
from .models import *
from .api import CiNiiURL, JALCURL, DATACITEURL
...
def get_cinii_data_by_key(api, keyword):
    ...
    # elif keyword == 'contributor' and data['items'][0].get['dc:organization']:
    #     result[keyword] = get_cinii_contributor_data(data['items'][0].get('foaf:organization'))
...
def get_cinii_numpage(startingPage, endingPage):
    ...
    # if data:
    #     return get_cinii_page_data(data)    
...
def get_jalc_data_by_key(api, keyword):
    ...
    # elif keyword == 'contributor' and data['items'][0].get['dc:organization']:
    #     result[keyword] = get_jalc_contributor_data(data['items'][0].get('foaf:organization'))
    # elif keyword == 'description' and data['items'][0].get('description'):
    #     result[keyword] = get_jalc_description_data(
    #         data['items'][0].get('description')
    #     )
...
def get_datacite_record_data(doi, item_type_id):
    ...
    if api_response["error"] \
            or not isinstance(api_response['response'], dict):
        return result
    # result = get_datacite_data_by_key(api_response, 'all')
...
def get_datacite_data_by_key(api, keyword):
    ...
    #     result[keyword] = pack_single_value_as_dict(data['items'][0].get('prism:number'))
    # elif keyword == 'pageStart' and data['items'][0].get('prism:startingPage'):
    #     result[keyword] = get_datacite_page_data(data['items'][0].get('prism:startingPage'))
    # elif keyword == 'pageEnd' and data['items'][0].get('prism:endingPage'):
    #     result[keyword] = get_datacite_page_data(data['items'][0].get('prism:endingPage'))
    # elif keyword == 'numPages':
    #     result[keyword] = get_datacite_numpage(
    #         data['items'][0].get('prism:startingPage'), data['items'][0].get('prism:endingPage'))
    # elif keyword == 'date' and data['data']['attributes']['dates']:
    #     result[keyword] = get_datacite_date_data(
