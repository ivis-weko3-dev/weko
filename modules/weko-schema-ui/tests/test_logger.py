...
from weko_schema_ui.logger import WEKO_SCHEMA_UI_MESSAGE, weko_logger
...
def test_message_resource():
    assert all([isinstance(v, dict) for v in WEKO_SCHEMA_UI_MESSAGE.values()])
    assert all(set(v.keys()) == {"msgid", "msgstr", "loglevel"} for v in WEKO_SCHEMA_UI_MESSAGE.values())
...
def test_weko_logger_key_exists(base_logger):
    ...
    param = WEKO_SCHEMA_UI_MESSAGE.get(key:='WEKO_SCHEMA_UI_FAILED_ADD_SEARCH_CONDITION')
...
def base_logger(mocker):
    app = Flask(__name__)
    mock_logger = mocker.patch("weko_schema_ui.logger.weko_logger_base")
