...
# .tox/c1/bin/pytest --cov=weko_indextree_journal tests/test_views.py::test_export_journals_exception -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-indextree-journal/.tox/c1/tmp
def test_export_journals_exception(app, db, client, users,test_journals):
    url = url_for("weko_indextree_journal.export_journals")
    login_user_via_session(client=client, email=users[0]["email"])
    if os.path.isfile("journal.tsv"):
        os.remove("journal.tsv")
