...
# .tox/c1/bin/pytest --cov=invenio_accounts tests/test_admin.py::test_roleview_get_query -vv -s --cov-branch --cov-report=html --basetemp=/code/modules/invenio-accounts/.tox/c1/tmp
def test_roleview_get_query(app):
    """Test RoleView.get_query method."""
    with app.app_context():
        ...
        view = RoleView(Role, db.session)
        # get_queryの実行
        query = view.get_query()
        result_names = [r.name for r in query.all()]
        # info_displayがNoneでないロールのみ取得できること
        assert "Contributor" in result_names
        assert "jc_xxx_groups_yyy" in result_names
        assert "jc_xxx_roles_contributor" not in result_names

# .tox/c1/bin/pytest --cov=invenio_accounts tests/test_admin.py::test_roleview_get_count_query -vv -s --cov-branch --cov-report=html --basetemp=/code/modules/invenio-accounts/.tox/c1/tmp
def test_roleview_get_count_query(app):
    """Test RoleView.get_count_query method."""
    with app.app_context():
        app.config.update(dict(
            WEKO_ACCOUNTS_GAKUNIN_GROUP_PATTERN_DICT=
            {
