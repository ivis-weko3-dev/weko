...
from weko_bulkupdate.logger import WEKO_BULKUPDATE_MESSAGE, weko_logger
...
def test_message_resource():
    assert all([isinstance(v, dict) for v in WEKO_BULKUPDATE_MESSAGE.values()])
    assert all(set(v.keys()) == {"msgid", "msgstr", "loglevel"} for v in WEKO_BULKUPDATE_MESSAGE.values())
...
def test_weko_logger_key_exists(base_logger):
    ...
    param = WEKO_BULKUPDATE_MESSAGE.get(None)

    weko_logger(None)

    mock_logger.assert_called_once_with(app=app, key=None, ex=None)
...
def base_logger(mocker):
    app = Flask(__name__)
    mock_logger = mocker.patch("weko_bulkupdate.logger.weko_logger_base")
