...
def test_process_item(app, db, esindex, location, db_itemtype,harvest_setting,db_records):
    with patch("weko_search_ui.utils.send_item_created_event_to_es"):
        ...
        with patch("invenio_oaiharvester.harvester.RESOURCE_TYPE_MAP",mock_resource_type_map):
            # jpcoar
    ...
    with patch('weko_search_ui.utils.send_item_created_event_to_es', return_value=None):
        ...
        _records = _etree.findall('./GetRecord/record', namespaces=_etree.nsmap)
        _counter = {}
        with patch("invenio_oaiharvester.tasks.soft_delete") as mock_delete:
            res = process_item(_records[0], harvest_setting[0], _counter, None)
...
def test_link_success_handler(app):
    with patch("invenio_oaiharvester.tasks.oaiharvest_finished.send") as mock_send:
...
def test_run_harvesting(app, db):
    with patch("invenio_oaiharvester.tasks.send_run_status_mail"):
        index = Index()
        db.session.add(index)
        db.session.commit()
...
def test_check_schedules_and_run(app,db):
    ...
    db.session.commit()
    with patch("invenio_oaiharvester.tasks.run_harvesting.delay") as mock_delay:
