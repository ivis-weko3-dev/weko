...
def test_link_success_handler(app):
    with app.test_request_context():
        with patch("weko_sitemap.tasks.sitemap_finished.send") as mock_send:
            data = [
...
def test_link_error_handler(app):
    with patch("datetime.datetime",**{"now.return_value":datetime(2022,10,2,1,2,3)}):
...
def test_update_sitemap(app,db):
    ...
    with patch("weko_sitemap.ext.WekoSitemap._generate_all_item_urls",side_effect=mock_gen_urls):
        ...
        with app.test_request_context():
            with patch("weko_sitemap.tasks.sitemap_page_needed.send") as mock_send:
