...
def test_ItemImportView_check(i18n_app, app,users, client,client_request_args):
...
def test_ItemBulkExport_index(i18n_app, users, client_request_args, db_records2):
    with patch("flask_login.utils._get_user", return_value=users[3]['obj']):
        with patch("weko_search_ui.admin.ItemBulkExport.render",return_value=make_response()) as mock_render:
            test = ItemBulkExport()
            res = test.index()
            assert res.status_code == 200
...
# .tox/c1/bin/pytest --cov=weko_search_ui tests/test_admin.py::TestItemBulkExport::test_check_export_status -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-search-ui/.tox/c1/tmp
    def test_check_export_status(self,app,client,users, redis_connect):

        with patch("weko_search_ui.utils.AsyncResult",side_effect=MockAsyncResult):
            with patch("weko_search_ui.admin.check_celery_is_run",return_value=True):
                with patch("flask_login.utils._get_user", return_value=users[3]["obj"]):
...
# .tox/c1/bin/pytest --cov=weko_search_ui tests/test_admin.py::TestItemBulkExport::test_cancel_export -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-search-ui/.tox/c1/tmp
    def test_cancel_export(self, app, client, users, redis_connect):
        ...
        with patch("flask_login.utils._get_user", return_value=users[3]["obj"]):
            with patch("weko_search_ui.admin.cancel_export_all",return_value=True):
                with patch("weko_search_ui.admin.get_export_status",return_value=(False,"","","","REVOKED",)):
                    res = client.get(url)
