...
def get_permission_filter(index_id: str = None, is_community=False):
    ...
    if index_id:
        ...
        if search_type == config.WEKO_SEARCH_TYPE_DICT["FULL_TEXT"]:
            ...
            if index_id in is_perm_indexes:
                ...
                should_path.append(dsl.Q("terms", path=term_list))
    ...
    if is_perm:
        ...
        if result:
            ...
            shared_user_match = dsl.Q("terms", weko_shared_ids=[user_id])
            shuld = []
...
def default_search_factory(self, search, query_parser=None, search_type=None, additional_params=None):
        ...
        def _get_keywords_query(k, v, params):
            ...
            if isinstance(v, str):
                ...
                if k == "title" and params.get("exact_title_match"):
                    should_list = []
                    should_list.append(dsl.Q("term", **{"title":kv}))
                    should_list.append(dsl.Q("term", **{"alternative":kv}))
                    qry = dsl.Q("bool", should=should_list, minimum_should_match=1)
                else:
                    split_text_list = _split_text_by_or(kv)
                    if len(split_text_list) == 1:
                    ...
                    if isinstance(vlst, list):
                        ...
                        for j in kvl:
                            ...
                            if j == "other" and k=="language":
                                ...
                                params = {"param1":vlst}
                                script = dsl.Q("script",script={"source":source,"params":params})
                                shud.append(dsl.Q("bool",filter=script))
                            else:
                                name_dict = dict(operator="and")
                                name_dict.update(dict(query=j))
                                shud.append(dsl.Q("match", **{key: name_dict}))
                                        ...
                                        if isinstance(alst, tuple):
                                            ...
                                            if len(split_text_list) == 1:
                                                ...
                                                for split_text in split_text_list:
                                                    ...
                                                    name_dict = dict(operator="and")
                                                    name_dict.update(dict(query=split_text))
                                                    should_list.append(dsl.Q("match", **{name: name_dict}))
                                                mut = []
                                                mut.extend(qt or [])
                                                qry = dsl.Q("bool", must=mut, should=should_list, minimum_should_match=1)
                                            shuld.append(
    ...
    def _get_file_content_query(qstr):
        ...
        if len(split_text_list) == 1:
            ...
            multi_cont_q = dsl.Q("bool", should=should_list, minimum_should_match=1)
